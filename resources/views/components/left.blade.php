<div class="w-1/3 border flex flex-col">
    ...
    <div class="bg-grey-lighter flex-1 overflow-auto">
        @foreach($customers as $customer)
            {{--                            <Link href="{{ route('messages',$customer->id) }}">--}}
            <{{ $as }} href="{{ route('messages',$customer->id) }}"  >
            <x-chatter-label id="{{ $customer->id }}" number="{{ $customer->unreadNotifications()->count() }}" bg="{{ $classes }}" fullname="{{ $customer?->fullname }}" time="{{ $customer->notifications()?->first()->created_at->diffForHumans() }}" message="{{$customer?->notifications()?->first()?->data['message']}}" />
            </{{ $as }}>
         @endforeach
    </div>
</div>
